...
function getToolLabel(tool: ToolInvocation): { label: string; dotColor: string } {
  const args = tool.args as { command?: string; path?: string } | undefined;
  const command = args?.command;
  const name = args?.path?.split("/").pop();

  if (!command) return { label: "Working...", dotColor: "bg-neutral-400" };

  if (tool.toolName === "str_replace_editor") {
    const dotColor = command === "create" ? "bg-emerald-500" : "bg-blue-500";
    ...
    const verb = verbs[command] ?? "Editing";
    return { label: name ? `${verb} ${name}` : `${verb} file`, dotColor };
  }

  if (tool.toolName === "file_manager") {
    const verbs: Record<string, string> = { delete: "Deleting", rename: "Renaming" };
    const verb = verbs[command] ?? "Managing";
    return { label: name ? `${verb} ${name}` : `${verb} file`, dotColor: "bg-amber-500" };
  }

  return { label: "Working...", dotColor: "bg-neutral-400" };
}
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation": {
                            ...
                            const { label, dotColor } = getToolLabel(tool);
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs font-mono border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    <div className={cn("w-2 h-2 rounded-full flex-shrink-0", dotColor)}></div>
